...
/* --- Preview panel toggle (right-side pane) --- */

void
nemo_window_set_show_preview_panel (NemoWindow *window, gboolean show)
{
    GtkPaned *paned;
    ...
    window->details->show_preview_panel = show;

    g_settings_set_boolean (nemo_window_state,
                            NEMO_WINDOW_STATE_START_WITH_PREVIEW_PANEL,
                            show);

    if (show) {
        /* Show preview panel: bring paned position leftwards */
        GtkAllocation alloc;
        gint total_width;
        gint preview_width;
        ...
        preview_width = window->details->preview_panel_width;
        if (preview_width <= 0) {
            preview_width = 280; /* default preview width in pixels */
            window->details->preview_panel_width = preview_width;
        }

        /* Only set position if the paned is already laid out.
         * If not (e.g. during initial window show), the default allocation
         * will give the preview child its minimum width naturally. */
        if (total_width > 100) {
            gtk_paned_set_position (paned, MAX (200, total_width - preview_width));
        }
        gtk_widget_show (gtk_paned_get_child2 (paned));
    } else {
        /* Hide preview panel: push position all the way to the right */
        GtkAllocation alloc;
        gint total_width;
        ...
        if (total_width > 100) {
            gint current_pos = gtk_paned_get_position (paned);
            window->details->preview_panel_width =
                MAX (10, total_width - current_pos);
        }

        gtk_paned_set_position (paned, total_width);
        gtk_widget_hide (gtk_paned_get_child2 (paned));
    }
}

gboolean
nemo_window_get_show_preview_panel (NemoWindow *window)
{
...
nemo_window_constructed (GObject *self)
{
	...
	/* Preview paned — splits the content area between
	 * file listing (left) and preview panel (right).
	 * The preview panel is always present; shown/hidden
	 * by adjusting the paned position. */
	{
		GtkWidget *preview_paned;
		GtkWidget *preview_scrolled;

		preview_paned = gtk_paned_new (GTK_ORIENTATION_HORIZONTAL);
		gtk_box_pack_start (GTK_BOX (vbox), preview_paned, TRUE, TRUE, 0);
		gtk_widget_show (preview_paned);
		window->details->preview_paned = preview_paned;

		hpaned = gtk_paned_new (GTK_ORIENTATION_HORIZONTAL);
		gtk_paned_pack1 (GTK_PANED (preview_paned), hpaned,
				 TRUE, FALSE);
		gtk_widget_show (hpaned);
		window->details->split_view_hpane = hpaned;

		/* Preview panel (right side, pack2 of preview_paned) */
		preview_scrolled = gtk_scrolled_window_new (NULL, NULL);
		gtk_scrolled_window_set_policy (
			GTK_SCROLLED_WINDOW (preview_scrolled),
			GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
		gtk_widget_set_size_request (preview_scrolled, 200, -1);

		window->details->preview_panel =
			nemo_preview_panel_new ();

		gtk_container_add (GTK_CONTAINER (preview_scrolled),
				   window->details->preview_panel);
		gtk_paned_pack2 (GTK_PANED (preview_paned),
				 preview_scrolled, FALSE, FALSE);

		/* Hide preview by default — will be restored from settings */
		window->details->preview_panel_width =
			g_settings_get_int (nemo_window_state,
			                    NEMO_WINDOW_STATE_PREVIEW_PANEL_WIDTH);
		if (window->details->preview_panel_width <= 0) {
			window->details->preview_panel_width = 280;
		}
		window->details->show_preview_panel = FALSE;

		/* Save preview panel width when the paned handle moves */
		g_signal_connect (preview_paned, "notify::position",
				  G_CALLBACK (preview_paned_position_changed_cb),
				  window);
	}
...
static void
preview_selection_changed_callback (NemoView *view, NemoWindow *window)
{
    NemoPreviewPanel *panel;
    GList *selection;

    if (window->details->preview_panel == NULL) {
        return;
    }

    if (window->details->show_preview_panel == FALSE) {
        return;
    }

    panel = NEMO_PREVIEW_PANEL (window->details->preview_panel);
    selection = nemo_view_get_selection (view);

    if (selection == NULL) {
        nemo_preview_panel_clear (panel);
    } else if (selection->next == NULL) {
        /* Single file selected — show its preview */
        nemo_preview_panel_set_file (panel, NEMO_FILE (selection->data));
    } else {
        /* Multiple files selected — clear preview */
        nemo_preview_panel_clear (panel);
    }
                                  ...
                                  NemoView *view)
{
    ...
    /* Connect preview panel selection tracking */
    g_signal_connect (view, "selection-changed",
                      G_CALLBACK (preview_selection_changed_callback),
                      window);
					 ...
					 NemoView *view)
{
	...
	g_signal_handlers_disconnect_by_func (view, G_CALLBACK (preview_selection_changed_callback), window);
}
...
nemo_window_show (GtkWidget *widget)
{
	...
	if (g_settings_get_boolean (nemo_window_state,
	                             NEMO_WINDOW_STATE_START_WITH_PREVIEW_PANEL)) {
		nemo_window_set_show_preview_panel (window, TRUE);
	}
